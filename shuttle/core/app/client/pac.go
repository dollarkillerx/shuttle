...
func PacProxy(domain string, pac bool) bool {
	if !pac {
		return true
	}
	...
	if ip != nil {
		...
		return !isChinaIP(domain)
	}

	if strings.LastIndex(domain, ".cn") == 3 {
		return false
	}
	...
	for _, v := range localNG {
		if strings.Contains(domain, v) {
			return false
		}
	}

	lookupIP, err := utils.LookupIP(domain, "127.0.0.1:5352")
	if err == nil {
		domain = lookupIP
		ip := net.ParseIP(domain)
		if ip != nil {
