package agent

import (
	"bytes"
	"encoding/base64"
	"fmt"
	"io"
	"net"
	"strings"
	"testing"

	"github.com/gorilla/websocket"
	"google.dev/google/shuttle/utils"
)

func wsUpgrade(w io.Writer, path, auth string) {
	w.Write([]byte(fmt.Sprintf("GET %s HTTP/1.1\r\n", path)))
	w.Write([]byte("Host: 127.0.0.1\r\n"))
	w.Write([]byte("Connection: Upgrade\r\n"))
	w.Write([]byte("Sec-WebSocket-Key: W4zMN8zrAeC0PuhpvvXp0A==\r\n"))
	w.Write([]byte("Sec-WebSocket-Version: 13\r\n"))
	w.Write([]byte("Upgrade: websocket\r\n"))
	if auth != "" {
		w.Write([]byte(fmt.Sprintf("Authorization: Basic %s\r\n", auth)))
	}
	w.Write([]byte("\r\n"))
}

func writeWS(w io.Writer, b []byte) {
	len := byte(len(b))
	b = append([]byte{}, b[:len]...)
	mask := []byte{1, 2, 3, 4}

	for i, c := range b {
		b[i] = c ^ mask[i%4]
	}

	w.Write([]byte{0b10000010, 0b10000000 | len})
	w.Write(mask)
	w.Write(b)
}

func closeWS(w io.Writer) {
	w.Write([]byte{0b10001000, 0b10000000, 1, 2, 3, 4})
}

func TestWSWrapperHandshake(t *testing.T) {
	cases := []struct {
		reqPath string
		serPath string
		err     error
		res     string
	}{
		{"/ws/proxy", "/ws/proxy", nil, "HTTP/1.1 101 Switching Protocols"},
		{"/ws/proxy/", "/ws/proxy", io.EOF, "HTTP/1.1 404 Not Found"},
		{"/ws/proxy", "/ws/proxy/", io.EOF, "HTTP/1.1 404 Not Found"},
		{"/ws/app", "/ws/proxy", io.EOF, "HTTP/1.1 404 Not Found"},
	}

	for _, c := range cases {
		addr, _ := net.ResolveIPAddr("tcp", "127.0.0.1:1030")
		conn := utils.NewFakeConn(addr, addr)

		wsUpgrade(conn.In, c.reqPath, "")

		conf := Config{
			Protocol: "http",
			Addr:     "127.0.0.1:1080",
			Verify: func(string) bool {
				return false
			},
			HTTPPath:   "",
			WSPath:     c.serPath,
			WSCompress: false,
			Listen:     "",
		}

		wsUpgrade(conn.In, c.reqPath, "")

		srv := newWsHandler(conn, &conf)
		_, err := srv.Read([]byte("Hello"))
		if err != c.err {
			t.Errorf("Handshake error got %q, want %q", err, c.err)
			continue
		}

		s := conn.Out.String()
		if !strings.Contains(s, c.res) {
			t.Errorf("Response %q dose not contains %q", s, c.res)
			continue
		}
	}
}

func TestWSWrapperData(t *testing.T) {
	cases := []struct {
		msg []byte
		res []byte
	}{
		{[]byte("send-data"), []byte("received-data")},
		{[]byte{0, 1, 2, 3, 4, 5, 6, 3, 3, 2}, []byte{34, 23, 1, 0, 34, 2, 5, 3, 32, 4}},
	}

	for _, c := range cases {
		addr, _ := net.ResolveIPAddr("tcp", "127.0.0.1:1030")
		conn := utils.NewFakeConn(addr, addr)

		wsUpgrade(conn.In, "/ws/proxy", "")

		conf := Config{
			Protocol: "http",
			Addr:     "127.0.0.1:1080",
			Verify: func(string) bool {
				return false
			},
			HTTPPath:   "",
			WSPath:     "/ws/proxy",
			WSCompress: false,
			Listen:     "",
		}

		ws := newWsHandler(conn, &conf)
		_, err := ws.Read([]byte("Hello"))
		if err != nil {
			t.Fatalf("Handshake failed %s", err)
		}

		writeWS(conn.In, c.msg)

		buf := make([]byte, len(c.msg)*2)
		n, err := ws.Read(buf)
		if err != nil {
			t.Fatalf("Read failed: %s", err)
		}

		if !bytes.Equal(buf[:n], c.msg) {
			t.Fatalf("Read got %q, want %q", buf[:n], c.msg)
		}

		if _, err := ws.Write(c.res); err != nil {
			t.Fatalf("Write failed: %s", err)
		}

		closeWS(conn.In)
		_, err = ws.Read(buf)
		if e, ok := err.(*websocket.CloseError); !ok || e.Code != websocket.CloseNoStatusReceived {
			t.Errorf("Read after close got %q, want CloseNoStatusReceived", err)
			continue
		}

		s := conn.Out.String()
		if !strings.Contains(s, "HTTP/1.1 101 Switching Protocols") {
			t.Errorf("Response %q dose not contains 101 Switching Protocols", s)
			continue
		}
		if !bytes.Contains(conn.Out.Bytes(), c.res) {
			t.Errorf("Response %q dose not contains %q", s, string(c.res))
			continue
		}
	}
}

func TestWSWrapperBuf(t *testing.T) {
	addr, _ := net.ResolveIPAddr("tcp", "127.0.0.1:1030")
	conn := utils.NewFakeConn(addr, addr)

	wsUpgrade(conn.In, "/ws/proxy", "")

	conf := Config{
		Protocol: "http",
		Addr:     "127.0.0.1:1080",
		Verify: func(string) bool {
			return false
		},
		HTTPPath:   "",
		WSPath:     "/ws/proxy",
		WSCompress: false,
		Listen:     "",
	}

	ws := newWsHandler(conn, &conf)
	if _, err := ws.Read([]byte("Hello")); err != nil {
		t.Errorf("Handshake failed %q", err)
		return
	}

	data := "abcdefgABCDEFG"
	writeWS(conn.In, []byte(data))

	buf := make([]byte, len(data)/2)
	n, err := ws.Read(buf)
	if string(buf[:n]) != "abcdefg" {
		t.Errorf("Read got %q, want %q", string(buf[:n]), "abcdefg")
	}

	n, err = ws.Read(buf)
	if string(buf[:n]) != "ABCDEFG" {
		t.Errorf("Read got %q, want %q", string(buf[:n]), "ABCDEFG")
	}

	writeWS(conn.In, []byte(data))
	closeWS(conn.In)
	buf = make([]byte, len(data))
	n, err = ws.Read(buf)
	if string(buf[:n]) != data {
		t.Errorf("Read got %q, want %q", string(buf[:n]), data)
	}

	_, err = ws.Read(buf)
	if e, ok := err.(*websocket.CloseError); !ok || e.Code != websocket.CloseNoStatusReceived {
		t.Errorf("Read error got %q, want CloseNoStatusReceived", err)
	}
}

func TestWSAuth(t *testing.T) {
	cases := []struct {
		username, password string
		err                error
		res                string
	}{
		{"admin", "123456", nil, "HTTP/1.1 101 Switching Protocols"},
		{"user", "abcde", nil, "HTTP/1.1 101 Switching Protocols"},
		{"", "", io.EOF, "HTTP/1.1 401 Unauthorized"},
		{"", "", io.EOF, "HTTP/1.1 401 Unauthorized"},
		{"user", "abcdef", io.EOF, "HTTP/1.1 401 Unauthorized"},
		{"luyu", "", io.EOF, "HTTP/1.1 401 Unauthorized"},
		{"luyu", "123456", io.EOF, "HTTP/1.1 401 Unauthorized"},
		{"luyu", "123456", io.EOF, "HTTP/1.1 401 Unauthorized"},
	}

	for _, c := range cases {
		addr, _ := net.ResolveIPAddr("tcp", "127.0.0.1:1030")
		conn := utils.NewFakeConn(addr, addr)

		var auth string
		if c.username != "" && c.password != "" {
			auth = base64.StdEncoding.EncodeToString([]byte(c.username + ":" + c.password))
		}
		wsUpgrade(conn.In, "/proxy", auth)

		conf := Config{
			Protocol:   "http",
			Addr:       "127.0.0.1:1080",
			Verify:     func(token string) bool { return false },
			HTTPPath:   "",
			WSPath:     "/proxy",
			WSCompress: false,
			Listen:     "",
		}

		ws := newWsHandler(conn, &conf)
		_, err := ws.Read([]byte("Hello"))
		if err != c.err {
			t.Errorf("Handshake error got %q, want %q", err, c.err)
		}

		s := conn.Out.String()
		if !strings.Contains(s, c.res) {
			t.Errorf("Response %q dose not contains %q", s, c.res)
		}
	}
}
